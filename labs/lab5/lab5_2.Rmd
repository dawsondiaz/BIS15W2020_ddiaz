...
```{r}
ggplot(data=iris, mapping=aes(y=Sepal.Width,x=Sepal.Length)) +
  geom_point()
...
ggplot will not do anything with NAs
```{r}
...
No y value because y is a count
```{r}
homerange %>% 
...
```{r}
mammals %>% 
  ggplot(aes(x=trophic.guild)) + geom_bar(stat="count")
```
...
```{r}
ggplot(data=smallest_10, mapping=aes(x=common.name, y=mean.mass.g))+geom_bar(stat="identity")+coord_flip()
```
