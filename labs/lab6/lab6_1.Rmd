...
```{r}
homerange %>% 
  select(taxon, trophic.guild, family, log10.mass) %>% 
  filter(trophic.guild == "carnivore") %>% 
  filter(taxon == "mammals") %>% 
  group_by(family) %>% 
  summarize(
    meanlog10mass = mean(log10.mass),
    minlog10mass = min(log10.mass),
    maxlog10mass = max(log10.mass),
    medianlog10mass = median(log10.mass))
```
...
ggplot(data = homerange, mapping = aes(x = log10.mass, y = log10.hra)) +
  ...
  labs(title = "Mass vs. Homerange",
       x = "Mass (log10)",
       y = "Homerange (log10)")+ 
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5, vjust = 2.75))
```
...
```{r}
homerange %>% 
  select(species, locomotion) %>% 
  group_by(locomotion) %>% 
  ggplot(mapping = aes(x=locomotion))+geom_bar(stat = "count") + 
  labs(
    title= "Count by Locomotion",
    x= "Locomotion Type",
    y= "Count") +
  theme(
    plot.title = element_text(size=rel(1.5), hjust=0.5, vjust=2.75))
```
...
```{r}
homerange %>% 
  select(class, taxon, species, thermoregulation) %>%
  ggplot((aes(x=thermoregulation, fill=thermoregulation)))+geom_bar(alpha=.5)+
  labs(
    title="Taxon by Thermoregulation",
    x="Taxon",
    y=NULL)+
    theme(
    plot.title = element_text(size=rel(1.5), hjust=0.5, vjust=2.50))
```
...
```{r}
homerange %>% 
  ggplot(mapping = aes(x= thermoregulation, y=log10.mass, fill=class))+geom_boxplot(alpha=0.2)+
  labs(
    title= "Thermoregulation by log10 mass and class"
...
You can group and fill by any other categorical veriable.
