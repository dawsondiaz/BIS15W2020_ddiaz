...
```{r}
homerange %>% 
  ggplot(aes(x=locomotion, fill=primarymethod))+geom_bar(position="dodge")
```

```{r}
homerange %>% 
  ggplot(aes(x=locomotion, fill=primarymethod))+geom_bar()+labs(
    title="Locomotion by Primary Method",
    y=NULL,
    x="Locomotion",
    legend.title="Primary Method"
      )+theme(
    plot.title = element_text(size=rel(1.25), hjust=0.5, vjust=2.50),
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.6, color = "grey")+
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="salmon", alpha  =0.6, color = "grey")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10.mass)) +
  geom_histogram(aes(y = ..density..), binwidth = .4, fill = "steelblue", alpha = 0.8, color = "grey")+
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.hra))+geom_histogram(color="grey")
```
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.hra))+geom_density(fill="black", color="grey")
```
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.hra))+geom_histogram(aes(y=..density..), color="green", fill="pink")+geom_density(color="red")
```
...
```{r}
homerange %>% 
  select(taxon, class, range_category) %>% 
  ggplot(aes(x=range_category, fill=taxon))+geom_bar(stat="count")
```
