...
tb_data %>% 
  pivot_wider(names_from = "key",
              values_from = "value") %>% 
  select(country, pop=population)
#you can rename in select
```
...
```{r}
msleep %>%
  group_by(genus) %>% #we are grouping by feeding ecology
...
There is no data entered for that observation
