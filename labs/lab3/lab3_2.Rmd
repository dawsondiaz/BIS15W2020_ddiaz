...
###^ select, in fish, any variables that have "length"
...
```{r}
fishdata <- select(fish, lakeid, length, scalelength)
```
...
```{r}
more_fishdata <- select(fish, -annnumber, -radii_length_mm, -fish_id)
```
...
###^ create a data fame you are interested in using select(). Then within that table you can refine your results using filter()
...
###^ in mac pipe is shift cmd m
###you can store a pipe in object
